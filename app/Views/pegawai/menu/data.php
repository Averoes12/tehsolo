...
<div class="card">
    ...
    <div class="card-body">
        ...
        <form method="POST" action="<?= base_url('pegawai/menu/data') ?>">
            <?= csrf_field(); ?>
            <div class="input-group mb-3">
                <input type="text" class="form-control" placeholder="Cari Nama Menu minuman" name="carimenu" autofocus
                    value="<?= $cari; ?>">
                ...
                if (count($datamenu) > 0) {
                    foreach ($datamenu as $row):
                        ...
                        <tr>
                            ...
                            <td <?= $row['stok'] < 10 ? "style='color: red;'" : "" ?>>
                                <?= ($row['nama_cabang'] == null || $row['nama_cabang'] == 0) ? "All" : $row['nama_cabang'] ?>
                            </td>
                            <td align="right">
                                ...
                                <a href="<?= base_url('pegawai/menu/edit/' . $row['id']) ?>" class="btn btn-primary btn-sm"> <i
...
<script>
        ...
        swal({
            ...
            .then((willDelete) => {
                if (willDelete) {
                    $.ajax({
                        type: "delete",
                        url: "<?= site_url('pegawai/menu/hapus/') ?>" + kode,
                        dataType: "json",
    ...
    $(document).ready(function () {
        $('.tombolTambah').click(function (e) {
            ...
            $.ajax({
                url: "<?= site_url('pegawai/menu/formTambah') ?>",
                dataType: "json",
                success: function (response) {
                    if (response.data) {
