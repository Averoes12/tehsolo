...
<div class="card">
        ...
        <div class="row justify-content-between">
            ...
            <form method="POST" action="<?= base_url('owner/transaksi/data') ?>">
                ...
                <div class="input-group mb-3">
                    <select id="type" name="type" class="custom-select custom-select-sm mr-3">
                        <option value="">Semua Tipe</option>
                        <option value="in" <?= $selectedType == 'in' ? 'selected' : '' ?>>Pemasukan</option>
                        <option value="out" <?= $selectedType == 'out' ? 'selected' : '' ?>>Pengeluaran</option>
                    </select>
                    <select id="branch_id" name="branch_id" class="custom-select custom-select-sm mr-3">
                        <option value="">Semua Cabang</option>
                        <?php foreach ($cabang as $branch) { ?>
                            <option value="<?php echo $branch['id'] ?>" <?= $selectedCabang == $branch['id'] ? 'selected' : '' ?>><?php echo $branch['nama_cabang'] ?></option>
                        <?php } ?>
                    </select>
                    <button class="btn btn-primary btn-sm" type="submit" name="filter">Filter</button>
    ...
    <div class="card-body">
        ...
        <form method="POST" action="<?= base_url('owner/transaksi/data') ?>">
            <?= csrf_field(); ?>
            <div class="input-group mb-3">
                <input type="text" class="form-control" placeholder="Cari Nama Menu" name="caritransaksi" autofocus
                    value="<?= $cari; ?>">
                ...
                if (count($datatrx) > 0) {
                    foreach ($datatrx as $row):
                        ...
                        <tr>
                            <td><?= $nomor; ?></td>
                            <td><?= $row['trx_date']; ?></td>
                            <td><?= ($row['nama_cabang'] == null || $row['nama_cabang'] == 0) ? "All" : $row['nama_cabang'] ?>
                            </td>
                            <td><?= $row['createby'] ?></td>
                            <td><?= $row['updateby'] ?></td>
                            <td><span
                                    class="badge <?= $row['type'] == "in" ? 'bg-success' : 'bg-danger' ?>"><?= $row['type'] == "in" ? 'Pemasukan' : 'Pengeluaran' ?></span>
                            </td>
                            <td><span
                                    class="badge <?= $row['cancelInd'] == "N" ? 'bg-success' : 'bg-danger' ?>"><?= $row['cancelInd'] == "N" ? 'Aktif' : 'Dibatalkan' ?></span>
                            </td>
                            <td align="right">
                                <?php if ($row['type'] == "in" && $row['cancelInd'] == "N") { ?>
                                    <button type="button" class="btn btn-danger" id="btn-cancel-trx-<?= $nomor ?>"
                                        data-id="<?= $row['id'] ?>" data-toggle="tooltip" data-placement="top"
                                        data-title="Batalkan Transaksi">
                                        <i class="fas fa-times "></i>
                                    </button>
                                    <a href="<?= base_url('owner/transaksi/generate/' . $row['id']) ?>" class="btn btn-warning"
                                        data-toggle="tooltip" data-placement="top" data-title="Cetak Transaksi"><i
                                            class="fas fa-print "></i></a>
                                <?php } ?>
                                <?php if ($row['type'] == "out") { ?>
                                    <a href="<?= base_url('owner/transaksi/editPengeluaran/' . $data) ?>"
                                        class="btn btn-primary btn-sm" data-toggle="tooltip" data-placement="top"
                                        data-title="Edit Transaksi"> <i class="fas fa-edit"></i></a>
                                <?php } ?>
                                <?php if ($row['type'] == "in") { ?>
                                    <button type="button" class="btn btn-success" id="btn-dtl-trx-<?= $nomor ?>"
                                        data-id='<?= $row['id'] ?>'
                                        data-cabang="<?= ($row['nama_cabang'] == null || $row['nama_cabang'] == 0) ? "All" : $row['nama_cabang'] ?>"
                                        data-toggle="tooltip" data-placement="top" data-title="Detail Transaksi"><i
